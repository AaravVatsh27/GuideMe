import { DEFAULT_TIMEZONE, MAX_PRICE, MIN_PRICE } from "@/server/constants";
import {
...
const yearOfStudySchema = z
  ...
  .int()
  .refine(
    (value) => YEAR_OF_STUDY_VALUES.includes(value as (typeof YEAR_OF_STUDY_VALUES)[number]),
    "Choose a valid year of study",
  );
const timeSlotSchema = z
  .string()
  .regex(/^([01]\d|2[0-3]):00$/, "Time must use HH:00 format");
...
const examEntrySchema = z.object({
  exam: examSchema,
  year: z.number().int().min(2000).max(currentYear + 1).optional().nullable(),
});
...
export const mentorExamsStepSchema = z
  .object({
    exams: z.array(examEntrySchema).max(EXAM_VALUES.length),
  })
  .refine(
    (value) => new Set(value.exams.map((entry) => entry.exam)).size === value.exams.length,
    {
...
export const mentorHelpStepSchema = z
  .object({
    specialisations: z.array(helpTopicSchema).min(1).max(5),
  })
  .refine(
    (value) => new Set(value.specialisations).size === value.specialisations.length,
    {
...
export const mentorPricingStepSchema = z
  .object({
    priceMin: z
      ...
      .max(MAX_PRICE)
      .refine(
        (value) => PRICING_POINTS.includes(value as (typeof PRICING_POINTS)[number]),
        "Choose one of the supported pricing points",
      ),
    priceMax: z.number().int().min(MIN_PRICE).max(999),
  })
  .refine((value) => value.priceMax === calculateFortyFiveMinutePrice(value.priceMin), {
    message: "45-minute pricing must be auto-calculated from the 30-minute session",
    path: ["priceMax"],
  });
...
export const mentorAvailabilityStepSchema = z
  .object({
    ...
    availabilitySlots: z.array(availabilitySlotSchema).min(5).max(98),
  })
  .refine(
    (value) =>
      new Set(
      ...
      ).size === value.availabilitySlots.length,
    {
...
export const mentorOnboardingSchema = mentorInstitutionStepSchema
...
export const mentorProfileUpdateSchema = z
  .object({
    ...
    expectedGraduationYear: z.number().int().min(currentYear).max(currentYear + 10).optional(),
    exams: z.array(examEntrySchema).max(EXAM_VALUES.length).optional(),
  ...
  .superRefine((value, context) => {
    if (
      ...
      value.priceMax !== calculateFortyFiveMinutePrice(value.priceMin)
    ) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        message: "45-minute pricing must be derived from the 30-minute price",
        path: ["priceMax"],
      });
    }
    ...
    if (
      value.exams &&
      new Set(value.exams.map((entry) => entry.exam)).size !== value.exams.length
    ) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        message: "Duplicate exams are not allowed",
    ...
    if (
      ...
      new Set(value.specialisations).size !== value.specialisations.length
    ) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        message: "Duplicate help topics are not allowed",
        path: ["specialisations"],
      });
    }
  });

export const mentorAvailabilitySchema = mentorAvailabilityStepSchema;
